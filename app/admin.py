...
from .models import env_info ,pet_info ,Schedule ,Tag_Info ,food_type,env_location

class env_locationAdmin(admin.ModelAdmin):  
    list_display = ('location_code','temperature','humidity','updated_at','created_at')
class env_infoAdmin(admin.ModelAdmin):
    list_display = ('location_name','location_code','updated_at','created_at')
    #後台要顯示的資料表格
...
class food_typeAdmin(admin.ModelAdmin):
    ...
    #後台要顯示的資料表格
admin.site.register(env_location,env_locationAdmin)
